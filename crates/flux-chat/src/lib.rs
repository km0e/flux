...
//! the WebSocket frame vocabulary — lives in `flux-session` (it spans
//! chats and sessions; none of it belongs to a single conversation task).
//! The two planes meet on narrow seams: this crate depends only on
//! flux-core ports (`OutputPort`); `flux-session` depends on this crate,
//! never the reverse.
...
pub use chat::{CONTEXT_BASE_KEY, ResolvedPin};
