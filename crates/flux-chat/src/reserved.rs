...
//! and the `question` tool. MCP registration must skip these names so an
//! external tool cannot shadow them: `register_if_absent` at assemble time
//! would keep the MCP entry and silently drop the chat-owned one.

use crate::domain::STATE_TOOL_NAMES;
use flux_core::{BUF_READ_TOOL, QUESTION_TOOL};

/// Whether `name` is a chat-owned tool MCP registration must skip.
pub fn is_reserved_tool_name(name: &str) -> bool {
    STATE_TOOL_NAMES.contains(&name) || name == BUF_READ_TOOL || name == QUESTION_TOOL
}
