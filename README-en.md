...
- **MCP client**: launch external MCP servers as child processes and expose their tools. Managed from the web UI's MCP dialog (stored in the server database; persist-first + LIVE apply — the manager spawns, registers the tools, and the matching chats rebuild at the round boundary), with a self-healing supervisor that respawns a dead child with capped backoff.
