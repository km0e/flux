...
function ToolChips({ server }: { server: McpServerSummary }): React.ReactElement {
  ...
  return <span className="text-2xs text-muted">{word}</span>;
}
...
function McpPreview(props: {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-border bg-panel p-4">
      ...
      <span className="text-2xs text-muted">Applies live — running conversations pick the tools up when their current round ends.</span>
    </div>
  );
}
    ...
    <MasterDetail
      list={
        <Rail label="MCP servers">
          ...
          {servers.length === 0 && (
            <RailEmpty>No MCP servers registered.</RailEmpty>
          )}
        </Rail>
      }
      detail={
