/**
 * stream-controller.ts — the per-chat StreamController class body.
 *
 * Split out of stream.ts (the facade keeps the public registry API and
 * re-exports this class). Reception and rendering are decoupled by exactly
 * one animation frame (P0-1): each delta only appends to the raw buffer
 * and schedules a coalesced re-render — at most ONE incremental render
 * runs per frame no matter how many deltas arrive. Without this, a burst
 * of deltas queues synchronous renders past the frame budget; the main
 * thread drops frames and the backlog paints in one visible surge
 * ("chunks suddenly appear"). Segment boundaries (tool cards, finalize,
 * dispose) flush synchronously — a closing segment must have its final
 * state in the DOM before the next one starts.
 *
 ...
 * Depends: core/state.ts, lib/markdown.ts, services/panes.ts, services/stream-state.ts,
...
import { controllers, disposeController, reasoningBlocks } from './stream-state';
...
export class StreamController {
  ...
  /** Append an error message to the pane (pane context — the retry/copy
   * actions are available; the retry refills the composer, never sends). */
  appendError(message: string): void {
    const pane = ensurePane(this.chatId);
    const el = createErrorBubble(message, { retry: true });
    pane.appendChild(el);
    this.scrollToBottom();
  }
