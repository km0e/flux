...
>
> 本文档描述 Flux 当前的真实架构——只写现在时；设计理由就近写在实现与代码注释里，明确接受的妥协（T-xx）见 [decisions.md](decisions.md)。所有图示使用 Mermaid（GitHub 与 VSCode Markdown 预览原生渲染）。

## 目录

- [系统总览](#系统总览)——一句话定位 · 总体设计 · 系统架构图
- [模块](#模块)——workspace 依赖图 + 每个 crate / 前端一小节（职责 · 协作）
- [跨模块机制](#跨模块机制)
  - 后端：核心概念 / 生命周期 / 对话循环 / 工具系统 / 工具上下文与边界 / 沙箱语义 / 持久化 / Connect 协议与背压 / 配置 / Web UI 静态伺服 / 终端侧信道
  - 前端：分层结构 / 状态管理 / 流式渲染 / 构建与测试
- [关键设计决策](#关键设计决策)——架构层设计立场索引；既定取舍（T-xx）见 [decisions.md](decisions.md)
- [开发与验证](#开发与验证)
...
- **一句话定位**：Flux 是一个通用编码 Agent 框架——Rust 编写的本地服务器负责 LLM 对话循环、工具执行与持久化，浏览器 Web UI（React + Tailwind，由 server 静态伺服）负责聊天 UI 与流式渲染。
- **总体设计**：client-server 架构，两者通过 Connect 协议（gRPC-Web）通信；每条对话跑「纯状态机 + 两通道泵」回路——client-server 让工具执行与持久化留在本地进程、UI 只做渲染；纯内核把全部 I/O 推给通道对端的适配器，换来确定性的对话语义、显式背压与可测试性（见「关键设计决策」）。
...
Cargo workspace 共 12 个 crate（含 dev-only 的 `flux-test-support`，不入下图），依赖方向严格自下而上；前端独立于 workspace。每个模块一小节：职责 = 它拥有什么，协作 = 它与谁交换什么（依赖方向见下图）。
...
- 职责：内核契约层——纯类型（`Message` / `Role` / `ToolCall` / `CoreError` / `ErrorCode` / `ChatStateKind`）、`WireEvent`（内核输出词汇）、内核 I/O 词汇（`loop_io.rs`：`LoopInput` / `LoopFact` + `RoundOutcome` / `StreamEvent` / `StreamHandle` / `Connection`）、`Tool` trait（`call(arguments, ctx: ToolCtx)`，每调用协作取消上下文 + `call_id` + 沙箱边界）+ `ToolRegistry`、边界解析（`boundary::resolve_path`，`ToolCtx::resolve` 的实现）、内核端口（`ToolPort` 工具执行 + `OutputPort` 适配器侧事件出口）、`Provider` 工厂（零 IO；依赖 serde、serde_json、strum、thiserror、async-trait、tracing、futures、tokio-util）。proto 契约（`flux.v1`）在 `flux-proto`（构建期生成），flux-session 的 router 是 WireEvent → 流元素的唯一映射点
- 协作：被其余全部 crate 依赖，自身无内部依赖——契约层单点

### flux-macros

- 职责：`#[derive(Tool)]` 过程宏：字段推断 JSON Schema + `call` 反序列化；`#[derive(ToolItem)]` 为嵌套参数项生成 `item_schema()`（对象数组字段）
- 协作：被需要派生工具 schema 的 crate 作为依赖使用（flux-tools 等）

### flux-proto

- 职责：`flux.v1` 契约（`proto/flux/v1`，buf 管理，STANDARD lint 为命名权威）——Rust 绑定由 `flux-proto` 构建期生成（tonic/prost，OUT_DIR），TS 绑定由 web 包 prebuild/pretest 钩子 `buf generate` 派生（不入库）；CI 重派生 + `buf breaking` + 新鲜度检查
- 协作：被 flux-session / flux-server 依赖（proto 流元素词汇）；TS 契约供 clients/web

### flux-provider

- 职责：OpenAI 兼容实现 + SSE 解析，实现 flux-core 的 `Provider` 会话工厂（实例按模型钉定；`begin` 开出 `Connection`）
- 协作：实现 flux-core 的 `Provider` 端口；由 flux-server 装配选用

### flux-store

- 职责：SQLite 持久化（sqlx，WAL：chats / messages / state / providers / mcp_servers）
- 协作：被 flux-chat（transcript / buf / state 持久化）与 flux-server（chats / providers / MCP 启动列表）依赖

### flux-tools

- 职责：内置工具：文件 / shell / 搜索 / Agent Skills（skill_list + 技能格式权威：发现 / 读取 / 目录节 / 激活格式化）+ 共享 subprocess 执行器
- 协作：工具经 `#[derive(Tool)]` 注册进 flux-chat 装配的 per-chat `ToolRegistry`；技能格式权威被 flux-chat 的 `skill_read` 消费

### flux-mcp

- 职责：MCP 客户端桥：连接外部 MCP 服务器——本地 stdio 子进程或远程 Streamable HTTP 端点——并暴露其工具（启动列表在 DB，UI 管理，persist-first + 即时应用；headers/env 值永不离开服务端）；接收 server 通知——`tools/list_changed` 走「通知钩子 + subscriptions/listen 订阅」双路径（覆盖 spec ≤ 2025-06-18 与 2026-07-28），`notifications/message` 日志转发到 UI
- 协作：由 flux-server 的 `McpManager` 管理；工具包装为 `McpToolWrapper` 进 chat 注册表；MCP 变更扇出引擎重建

### flux-loop

- 职责：对话内核：纯状态机（`machine.rs`）+ 纯泵驱动（`runtime.rs`：消费输入、步进、按序转发事实）——零 I/O、零 trait 对象
- 协作：纯内核——由 flux-chat 的 spawn / round 消费（装配内核 + 折叠事实）

### flux-chat

- 职责：会话层·数据平面：单 chat 任务机制（`chat` 实体 / `domain` 状态与 state 工具 / `handle` 控制句柄 / `spawn` 装配 / `round` 轮次消费者（事实 fold + 飞行监督；Rebuild → 机器门原地重建，任务不退出）/ `tool_exec` 飞行监督库（无独立任务）/ `buf` 溢出缓冲（store 支撑）/ `skills` chat-owned `skill_read`（激活去重）+ 技能目录合成 / `question` 提问工具 / `reserved` 保留工具名检查）。依赖 flux-core 端口（`OutputPort`）与 flux-tools 的技能格式权威，对控制平面无反向依赖
- 协作：数据平面——被 flux-session 缓存与调度（`CachedChat.task`）；对控制平面无反向依赖

### flux-session

- 职责：会话层·控制平面：跨 chat / 跨 session 的簿记——`manager`（`ServerState`：全局配置 + chat 缓存 + 身份注册表）/ `ops`（租约·订阅·广播）/ `router`（WireEvent → proto 流元素的映射 + fanout）/ `lifecycle`（懒 spawn 与任务替换）/ `identity`（会话标识·类型化 sink）
- 协作：控制平面——被 flux-server 装配；持有 chat 缓存、身份注册表与 router

### flux-server

- 职责：TCP 传输（Connect 面 `/flux.v1.*` + 终端侧信道 `/ws/term`）、`grpc/*`（chat/事件/管理/fs 服务的序列化薄层）、装配、CLI（无配置文件：flag + `FLUX_*` 环境回退 + `--generate-completions` shell 补全）、`ProviderRegistry`（provider 管理：选择 / 实例构建 / 模型探测 / 端点编辑（UpdateProvider——persist-first 换 slot，随后重解析钉住对话的实例，活引擎在门处热应用），实例递入 chat 层）、`McpManager`（MCP 启动列表管理：persist-first + 即时应用）
- 协作：装配根——组装以上全部 crate；对外暴露 Connect 面 `/flux.v1.*`、终端侧信道 `/ws/term` 与静态站

### flux-test-support

- 职责：dev-dependency 专用测试支撑——`test_env_guard!` 宏（pre-main ctor 剥离 `http_proxy` 等代理变量与 `FLUX_*` 宿主变量：防代理劫持 loopback 请求、防环境变量污染测试断言）+ `loopback_client`（代理免疫的 loopback 客户端）
- 协作：只允许出现在各 crate 的 `[dev-dependencies]`——守卫改进程全局态，绝不进生产构建

### clients/web

- 职责：Web UI（唯一前端）：React 19 + Radix UI + Tailwind v4 + zustand，Vite 构建
- 协作：TS 契约由 proto 经 `buf generate` 派生；经 Connect 面（gRPC-Web）同源回连 server——内部分层与机制见「跨模块机制 → 前端」各节

## 跨模块机制

### 核心概念：ServerState / Chat / Session
...
- `ChatInfoOwned` / `ChatInfoGuard`：缓存快照零拷贝设计——先取下锁快照、释放读锁，再序列化发送，**读锁绝不跨 I/O 或网络发送持有**。快照携带两个全局状态位：`active`（租约被持有）与 `running`（live 任务有轮次在飞——真值是任务 round-state watch 槽；任务 mid-round 死亡时 done 过滤保证不产生幽灵 running）。`chats` 广播在租约转换之外也随**轮边界**重发（lifecycle 的 watcher 订阅引擎 round-state watch，Idle↔Streaming 每次翻转即广播；任务死亡补一次终值广播）——侧边栏的 running 标记因此是跨窗口真值，而非 viewer 局部流状态。
...
- **连接**（`flux-provider`，`Connection` trait 在 flux-core）：每 chat 一个有状态会话（prefix cache），单接口——`open(pending, sink) → StreamHandle`（解析后的事件推进回路输入通道；stall 看门狗与 EOF 截断判定都在连接内）。连接活到下一次机器门触发——真相源变更时消费者在门处**原地 re-begin**（新连接，旧连接随替换 Drop）。SSE 的 tool-call 分片按 wire `index` 累积，累积器**绝不按 index 自扩容**——≥ `MAX_TOOL_CALL_INDEX`（128；请求 n=1，多 call 响应也只是个位数）的索引连同其分片直接丢弃，warn-ONCE 闩保证碎片洪水不淹没日志；16 MiB 字节上限只约束字节，管不住按 index 的内存放大（恶意/损坏上游一个几字节的 `{"index": 10⁹}` 一步分配数十亿条目）。
...
- **引擎重建（通用重启原语，机器门）**：内核的全部认知收缩为一个门原语——`Hold`（Idle 立即触发并发 `GateReleased`——门触发即排空：决策前发送的一切都按 FIFO 先于 Hold 入队；活轮次武装到收尾）与 `GateReleased`（门触发的事实；队列排空后门才触发，排队轮先跑完）。选择与语义全在轮次消费者（见本节「轮次消费者」bullet）：变更方（session 层的 SwitchProvider / MCP 管理 / provider 端点编辑——UpdateProvider 换 id 背后的 url / api key 并重解析钉住对话的实例）**先写真相源**（请求时持久化 pin、变更全局注册表）并公告 wire 通知，再 `Rebuild`（provider 切换与端点编辑携带新实例）；消费者在门触发处**原地重建**——以与 spawn/hydration 同一条装配路径重装注册表与连接（全量持久历史 + 携带实例 + 当前全局注册表），随后注入待写入的 follow-up。活轮次永不中断；变更与生效解耦；引擎永不因重建而死亡，重建期间到达的发送直接入内核队列（旧连接上跑完，语义 FIFO 自洽）。**门触发即丢弃 pending**（两个触发点同规：Hold 到达于 Idle、武装门在收尾）——中断残留（被 void 的排队工具结果 + 在飞工具的部分输出）由重建前缀携带：门只在 Idle/收尾触发，彼时 transcript 刚提交完，全量持久历史已包含全部 pending 材料；下一轮若重发即重复工具消息（浪费 token、破坏前缀缓存、严格端点 400）。取消前已排队的轮次仍投递残留——它们跑在重建前连接上，其前缀确实缺这些。**从某条消息重启对话 = fork**（见 ChatService）：新会话复制源 transcript 至该用户消息**之前**（重做轮在用户重发时才回到 fork——客户端把其内容预填进 fork 的输入框），源原样不动——没有原地归档；
...
- **schema/解析单源**：内置工具都是字段结构体，宏从字段（含 doc comment）推断 JSON Schema，`call` 自动反序列化参数（失败 → `CoreError::InvalidArguments`）并透传 `ToolCtx`；`#[tool(skip)]` / `#[tool(required)]` / `Vec<T>` 推断保留；`#[serde(default)]` 字段不进 `required`。`Vec<Item>` 的项类型为非原始类型时 `items` 委托给 `<Item>::item_schema()`（`#[derive(ToolItem)]` 生成，缺 derive 编译失败）——`read_file` 的 `ranges` 与 `edit_file` 的 `edits` 对象数组参数即此机制；项内 optionality 只用 `Option<T>` 表达（item 字段不支持 `#[serde(default)]`）。**单文件多操作** fs 工具语义：一次调用一个文件，路径只写一次——`read_file` 省略/空 `ranges` 读整文件，多窗口（≤ 16）各自保留 footer，≥2 窗口才加 `=== path ===` header；`edit_file` 1..=16 条 edits 顺序应用（后项可匹配前项产出）且**整调用原子**——edits 全部作用于内存串、文件末尾一次性写盘，任一 edit 失败该文件不落盘并报出失败的 edit 序号。
...
- **搜索 = rg 的库化拆分（grep-* + ignore）+ 批量调用**：grep 用 `grep-regex`（匹配）+ `grep-searcher`（逐行、行号、二进制 NUL-quit、上下文行）+ `ignore` 并行 walker（.gitignore/.ignore/.git/info/exclude 语义、`require_git` 默认、`hidden(false)` 保持搜点文件、`follow_links(false)`——比旧手写 walk 收窄：symlink 目录不进入）；边界仍只在入口 `ctx.resolve` 一次。一次调用 1..=8 个 pattern：单 pattern 输出保持扁平 `path:line: text`（老格式）；多 pattern 按 pattern 分组、每 pattern 200 行软配额（热 pattern 不饿死兄弟）、全局 500 行硬上限，上下文行（`context` 0-5，`path-line- text`）计入配额；单 pattern 编译失败渲染为该组错误段（其余照常），全失败才 Err；结果按 (路径， 行号， 文档序) 排序——并行遍历序不定，排序保确定性。`include` 参数走 OverrideBuilder（gitignore 式 glob 过滤）。行为规格：显式**文件**参数不受 ignore 限制、**目录**参数受（rg 语义；搜 ignore 目录走 bash）。glob 批量化 `patterns: 1..=8` 但不换 walker（`walk_dir` 保留）：upfront 编译任一非法即 Err（扁平列表无错误段槽位，与 grep 的隔离策略是有意的非对称），跨 pattern 首中即得去重。既有防护保留：16MB 文件跳过、单行 500 字符 match 窗口（`truncate_around_match`，Sink 内调用）。
- **Agent Skills（目录快照进 prompt + 工具式渐进披露）**：skill = 自包含能力包（含 `SKILL.md` 的目录：frontmatter `name`/`description`，正文为指令）。三个消费面共享同一发现与格式权威（flux-tools::skills）：① **Tier-1 目录**——`spawn` / `apply_rebuild` 两个 begin 点把目录节（name + source + description；单条截 1024 chars、总预算 8000 chars、溢出「…N more」尾注；空目录整节不出现）追加到 preamble + workdir 行 **之后**（前缀缓存保持 preamble 字节稳定；workdir 行是 per-chat 常量、终生不变，故排在易变的目录之前——见「沙箱语义」）；目录是 begin 点的**快照**，节内文案显式声明并指向 `skill_list`；② `skill_list` 每次调用即时扫描（无重启语义）——快照的活刷新口与会话中途装卸 / 预算截断 / 名字幻觉的自纠错口（全局技能在边界外，它是模型唯一的全局枚举口）；③ `skill_read` **chat-owned**（flux-chat::skills，同 buf_read 的注册模式，reserved 名单）——**激活去重由 transcript 派生**：两个组装点从 history 派生已激活索引（键 = 名字 + 词法规范化 rel；溢出判别 = 结果字符数 > INLINE_BUDGET，溢出内容经 buf_entries 回读），命中（规范化内容 hash 相同）返回短注不重注（溢出过则附 buf_read ref），未命中返回全文并记账——**不建任何新存储**，fork / 重建 / 重启随重派生天然正确（store 是唯一真相、读侧派生，同 validate_history）。内容同一性单源 = `skill_content`：SKILL.md = 剥 frontmatter 的正文（元数据在 discovery 期已消费——只编辑 frontmatter 仍算命中），其他文件 = 原文；激活返回 = `<skill_content name>` 包装 + 资源清单（浅层 symlink 安全 walk、20 条封顶、只列路径不急切读；不返回技能根绝对路径——按名寻址契约不需要）。位置：项目 `<workdir>/.flux/skills/`（边界内）+ 全局 `~/.flux/skills/`（用户安装的可信内容，同 MCP 服务器的信任层级）；同名时项目覆盖全局。`skill_read` **按名寻址**——模型永不传路径，请求文件相对技能根做严格包含检查（canonicalize + 前缀，symlink 安全），读取结构上不可能离开技能目录；
- **输出溢出缓冲（集中式、锚定、持久化）**：所有工具结果经 `Chat::bounded_output` 统一截断——超 8000 chars **写穿透传**至每-chat `buf_entries` 表（按产生它的 tool call id 锚定，`ToolCtx::call_id`），返回 head + 该 call id 引用；模型用 `buf_read {ref, offset, limit}`（字符制分页，页 ≤ 6000 chars，无递归；读穿 store）读取余下。**从不覆盖、无代际清空**：引用自描述且稳定（transcript 里的就是同一 id），跨引擎重建与进程重启可读，无需任何外壳传递（内存缓冲已删，store 是唯一真相）；生命周期 = chat 生命周期（transcript 只增不减，无归档边界，无需 GC）；fork 复制其副本携带的调用条目，`buf_read` 引用在 fork 内继续可读；chat 删除级联。entry ≤ 1M chars。per-tool 上限并入集中层：bash 8KB / read_file 行长与总量截断删除；grep 保留匹配窗口塑形 + 行数上限（单 pattern 500 行；多 pattern 每 pattern 200 行、全局 500 行，上下文行计入）；glob 500 条。grep 路径以搜索根为基准；read_file 页脚 `end` 为最后展示行（含），恰好剩 `limit+1` 行时必触发；
- **MCP**：自由函数 `connect_with_peer` 连接外部 MCP 服务器（`McpServerConfig` 枚举：`Stdio` 子进程 / `Http` Streamable HTTP 端点——rmcp `StreamableHttpClientTransport`，自定义 header 携带鉴权，`allow_stateless` 接受无会话服务器、`reinit_on_expired_session` 在 404 会话过期时传输层内自愈重握手；代理随进程环境 `http_proxy/https_proxy/all_proxy` 自动生效（loopback 目标豁免——`127.0.0.1`/`localhost` 是本地服务，与 stdio 子进程同层，绝不绕道代理）；30s 初始化超时），获取其工具列表包装为 `McpToolWrapper`（单次调用 60s 超时）；`McpSession` 为 RAII 保活守卫（两传输共用，`QuitReason::Closed` 在 HTTP 下即连接断开，由同一 supervisor 退避重连——传输层内的 SSE 重试/会话恢复是内圈自愈，supervisor 是外圈）。supervisor 以**条目代（generation）**自守——每次 connect（restore / add / respawn）铸造单调 generation，与 entry 及其 supervisor 任务成对登记，每个改 entry 的步骤在锁下复查该对：退避唤醒发现 entry 已移除或被替换 → 旧 supervisor 退出；respawn 采纳时被取代 connect 的会话**整体 Drop**（杀掉重复子进程）而非覆盖其上；`tools/list_changed` 通知携带 generation——陈旧信号无法在当前集合之上重复注册（注册前也复查 entry 仍存在，消失的条目不再泄漏注册项）。remove→re-add 竞态（退避睡眠窗口内 id 被删又加）因此结构性地不可能再产生重复会话、工具集被清空或取消指错会话。启动列表在服务端数据库（UI 管理，persist-first + 即时应用：`McpManager` 持全局注册表引用——连接成功即注册、移除即按 owner 精确注销，随后扇出引擎重建；连接失败随 ack 内联、行保留，启动时以 warn 跳过、不阻塞——管理入口始终可用，可修改后重试）。
...
### 沙箱语义：workdir / current_dir

- **`workdir` = 沙箱边界（只读，非广告 state）**：chat 创建时携带、canonicalize 后持久化进 state 表（`list_chats` join 它下发给 `ChatInfo.workdir`）；`StateManager` 加载时把 `workdir` 提取为**固定字段**，可变 map 中不存在——`set("workdir", …)` 在唯一写入点直接拒绝。**值进 system prompt**：`compose_system_prompt` 在每个 begin 点把 workdir 行合成进系统提示（稳定在前——边界终生不变，技能目录在后随重建刷新），这是路径唯一的常显面；state schema **不再列出该键**（列出来只会教模型为 prompt 已有的事实花一次 `state_get` 往返），隐藏读 `get("workdir")` 仍服务固定字段；跨项目 = 开新 chat。
...
数据库内的实体经 UI 管理（Connect RPC，失败内联返回，成功广播）：**Provider 注册表**（Providers 对话框；纯端点 id/url/api_key——不带 model，模型是 CreateChat / SwitchProvider 的必填钉定；api_key 永不下发；条目可编辑——`UpdateProvider` 在固定 id 背后换 url / api key（id 是对话钉定与已存模型的引用键），api_key 三态：缺省保留现值（它没离开过服务端，无值可回传）、present 替换、空串清除；persist 后广播并重解析钉住对话的实例，活引擎在门处热应用，不发 `provider_switched`——pin 没动）与 **MCP 启动列表**（MCP 对话框；**persist-first + 即时应用**——连接成功注册进全局注册表并扇出引擎重建，spawn 失败的行保留、下次启动重试；env 值只存不下发）。超时（connect/read 各 30s）为常量：read 超时逐读 idle 兜底，活着的长 SSE 流不被总时长误杀。
...
graph TB
 ...
 GRPC["grpc.ts + grpc-connection.ts（连接管理）+ grpc-frames.ts（纯翻译）+ grpc-send.ts（发送路径）— Connect 客户端"]
 ...
 DOML["dom.ts — 消息气泡 / 思考块 / 错误构建；toolcard.ts — 工具卡族（图标/摘要/卡片/判定）；follow.ts — 滚动跟随"]
 ...
 STREAM["stream.ts（门面/注册表）+ stream-controller.ts（控制器本体）+ stream-state.ts（共享注册表）— StreamController（分段状态 + rAF 合帧渲染）"]
 TS["transcript-search.ts — 会话内检索（跨节点匹配 + CSS Highlight API，零 DOM 触碰）"]
 CMD["commands.ts — 命令面板注册表（chats + actions 单一真相；selectChat 共享路径）"]
 MENTION["mention.ts — composer @ 路径补全（TTL 目录缓存 + 相对 workdir 插入）"]
 THEMESVC["theme.ts — 主题应用（data-theme + 终端调色板重读；选择住在 store）"]
 ...
 HOOKS["useEscapeKey"]
 PRIM["ui/ — 控件原语（button · fields · badge · spinner · skeleton，样式权威）<br/>+ Radix 封装（dialog · dropdown-menu · tooltip · tabs，shadcn 惯例）"]
 COMP["App · TopBar · Sidebar（Radix Tabs + 搜索过滤 + 行内改名 + 行操作菜单）<br/>ChatView · MessageList · TranscriptSearchBar · CommandPalette · ShortcutsDialog · ChatInput · UsageStats<br/>Explorer（react-arborist + 文件图标）· RightDock（多文件 tab + 终端 tab）· FileTabView · TerminalPanel · FileIcon · ErrorBoundary · Toasts（统一报错）<br/>dialogs/ — registry 注册 + ConfirmDialog + NewChatDialog（目录浏览器 + kind 选择）+ QuestionCard<br/>settings/ — SettingsDialog（单对话框三分节）+ Providers / MCP / Skills 面板"]
 end
 SVC --> LIB
 SVC --> CORE
 DISP --> SH
...
- 流式期间**绕过 React 直接命令式改 DOM**（性能边界，见「前端：流式渲染」）；
...
- **控制原语层**：`components/ui/`（index barrel 汇出 Button/IconButton/TextField/SelectField/TextArea/Badge/Spinner/Skeleton；行为组件零手写的 Radix 封装同住该目录）是全部控件的唯一样式权威——只消费 `--fx-*` token 与标尺（`--fx-radius-*`、`--fx-control-h`）；全局 `:focus-visible` 焦点环 + 覆盖滚动条统一视觉；
...
- **可折叠侧栏**：Ctrl/Cmd+B + 栏内 toggle；移动档（<768px，唯一断点）转抽屉（backdrop 关闭、选中自动收起、**Escape 关闭**——抽屉是最顶层 surface，先于 round 取消；打开时主列与右坞 **`inert`**（`useCoveredByDrawer`：键盘/AT 焦点不落到覆盖层后面，TopBar 的开关保持可达））；桌面拖拽或 separator 方向键 ±24px 调宽 160–360px 持久化（两块可拖 pane **共享对话列地板**——`core/prefs.ts` 的 `CONVERSATION_MIN_WIDTH` 280px：`clampSidebarWidth` 知坞宽、`clampPreviewWidth` 知侧栏，平板宽度（768–1024）下两者不再能联手把对话列挤到地板之下；存量偏好不动，只在渲染时让步）；**宽度权威在 app.css 的外壳布局段**（`#sidebar { width: var(--fx-sidebar-w) }` + `overflow: hidden`）——tab 切换/新建对话/树加载永不反推宽度（app.test 钉住）；拖动期间宽度直接走 CSS var（pointermove 零 React 渲染），pointerup 才提交 store + 持久化，pointercancel 提交最后移动宽度并拆卸（cancel 事件坐标不可信）；拖拽与键盘共用同一 spec（`useEdgeResize`/`edgeResizeKeys`），separator 暴露 splitter ARIA（tabindex + `aria-valuenow/min/max`）。新聊天按钮 + 客户端过滤（name/workdir 子串）；行操作（改名/删除）收敛为 Radix DropdownMenu 的 ⋯ 菜单（危险项红色）；改名走行内编辑器（受控 input，Enter/失焦提交、Esc 还原）；**行内运行标记**——标题前的 accent Spinner，取 wire `running`（chats 广播的轮边界真值，跨窗口可见）与本地 `streaming`（发送即亮）的或，行可访问名随之追加 ", running"；
...
- **停靠式，推动对话列**：body 行内的 flex 兄弟项而非覆盖层——拖宽即把对话列推走，永不遮挡内容；持久化宽值受渲染期 **`min()` 守卫**（`100vw − sidebar − 对话列地板`）钳制——这是拖拽钳制当不了的层：窗口每次变化都要重算，而拖拽钳制只在拖动时生效（宽屏存下的坞宽落到窄窗口同样被钳住），移动档媒体查询翻成**全屏 sheet**（<768px 唯一断点；持久化桌面宽度被中和）；左缘拖拽手柄（window pointer 监听 + `body.resizing-preview` 禁选中 + 松手持久化；separator 方向键 ±24px）；文件正文恒 `white-space: pre` 横向滚动；
- **终端 tab（可多个，“+”/空态动作按需创建）**：终端字体内置（JetBrains Mono + Nerd Font Mono 图标补丁**全量**，OFL-1.1——`styles/fonts.css` `local()` 优先、图标按字形需求加载，`scripts/fetch-fonts` 更新；该文件同时声明 UI 界面字体 IBM Plex Sans）；会话存于 `services/terminal.ts`（Map per tab），切 tab/chat 卸载面板但 PTY 与 xterm 缓冲继续；`sessionStorage` 按 chat 记住终端 id 列表，刷新后恢复全部 tab 并在宽限期内重连同一 PTY（服务端回放 256KB scrollback）；socket 自愈——任何异常断链（后端重启含在内）进入 1s→2s→4s→5s 封顶退避重试，stale term id 由服务端回落全新 spawn，killed/exited 两种终态除外；tab 独立关闭（杀各自 PTY）；shell 干净退出（code 0）自动关闭其 tab（PTY 已被服务端拆除、退出是用户主动行为），失败退出（非 0）保留 tab 与退出码状态行供排障；主题经 `html[data-theme]` MutationObserver 从 `--fx-*` token 重读；
- **会话内检索（Ctrl/Cmd+F）**：`services/transcript-search.ts` 在活跃 pane 的**已渲染 DOM** 上做跨节点大小写不敏感匹配（文本节点拼接 + 绝对偏移单调映射），命中经 **CSS Custom Highlight API**（`::highlight(flux-search[-current])`，stream.css）从外部上色——流式 append-only DOM **零触碰**；无该 API 的引擎不拦截 Ctrl+F（原生查找保留）。流式增量经 150ms 防抖 MutationObserver 重扫（只重计数**不重滚**——视图不被流式输出拽走）；Enter/Shift+Enter 环绕导航；Esc 关闭先于轮次取消（与移动抽屉同一分层规则）；查询跨开关保留（find-bar 记忆）；检索范围 = 已渲染转写，历史尾分页的更早页加载后进入索引；
- **命令面板（Ctrl/Cmd+K）**：`services/commands.ts` 注册表 = chats + 应用动作的**单一真相**——每个 run 委托既有服务入口（startNewChatFlow / addTerminalTab / openSearch / cycleTheme / bridge.reconnect），绝不复写流程；`selectChat` 由侧栏行与面板共享（activeChatId 变更 → mount → chat_claim，无独立 chat_open；移动档选中即收抽屉）。组件是 Radix Dialog + 真实 listbox 语义（input combobox + `aria-activedescendant`，↑/↓ 环绕、Enter 运行即关闭、点击同行）；查询**一次性**——重开清空（vs 检索栏的 find-bar 记忆）。面板暴露了两处被组件本地 state 囚禁的全局状态并随之上浮：主题选择入 store（`services/theme.ts` 承接 data-theme 翻转 + 终端调色板重读——store action 不触 DOM），settings 开关 + 「开到指定分节」的一次性意图（`SettingsTab` 下沉 core/types）同入 store；动作可用性按注册表门控评估（reconnect 仅断连时、search/new-terminal 需活跃会话、search 另受 Highlight API 门——与 Ctrl+F 拦截同一条门）；
- **Composer 文件引用（@-mention + 拖放插入）**：`@` 唤起 `services/mention.ts` 的路径补全弹层——workdir 相对路径、`/` 钻取（选目录保留 `@` 并续入下一段，选文件消费 `@` 并尾随空格）、目录优先、段内大小写不敏感匹配、上限 12；目录列表按会话缓存 + **15s TTL**（与 Explorer 刷新同律——agent 刚创建的文件立即可引用），与 Explorer 树状态互不依赖（Files 标签从未打开也能补全）。弹层是 composer 局部面：↑/↓ 环绕、Enter/Tab 插入、**Escape 就地关闭**（stopPropagation 屏蔽应用级链——关弹层绝不取消轮次）、mousedown-before-blur 保持焦点；输入中 IME 合成期的 Enter 确认候选而非插入。Explorer 行是拖拽**来源**（text/plain 携绝对路径，composer 落点按 chat workdir 相对化——界外路径保持绝对、由工具边界诚实拒绝），树自身不接受落点；
- **错误卡片与快捷键速查表**：pane 内错误（`stream.ts` 的 appendError）渲染为危险色小卡，附两枚安静 token 控件——**Retry** 回溯最近 user 气泡、经共享 `flux:compose` 桥把该轮原文回填 composer（**只回填不重发**：失败轮留在转写里，是否再叠一轮由用户决定——T-08 手动切分的取舍），**Copy** 携带错误原文；无会话的裸错误（`dispatch.ts`）无动作可附。速查表（裸界面 `?` 键 / 面板「Keyboard shortcuts」动作）的 Application 行由注册表自身的 `shortcut` 字段经 `listShortcutRows()` 派生（和弦与动作永不漂移），Composer 行（Enter / Shift+Enter / Escape）归组件静态持有；输入目标保留字面 `?`；
...
核心不变量（`services/stream-controller.ts` StreamController + `lib/render.ts`）：
...
- **工具卡判定声部**：结果以**纯字符串**上 wire（无错误旗标），客户端按内核自己的字符串标记分类判定（`lib/toolcard.ts` classifyToolResult）——`Error: ` 前缀（工具 Err/崩溃，flux-chat chat.rs/tool_exec.rs）→ danger 色轨 + 完整不淡出；`[interrupted by user]` 中断标记（flux-core）→ warn 状态语；行尾 `(exit code: N)` 非零后缀（flux-tools format_command_output 对有输出的失败命令补记）→ `exit N` warn 状态语。判定只上色卡片摘要，不改写结果文本（文本即真相；成功命令 echo 出标记前缀的误报面已文档化并接受）；
...
- **滚动跟随（粘滞状态机 + rAF 合并）**：渲染帧内 `scheduleFollow`——一帧至多滚一步（behavior auto）；粘滞中**上滚且已离开底部**（2px 容差）才脱离并取消挂起帧（阅读不被拽回）——钉在底部时内容收缩引发的 clamp（scrollTop 被浏览器拉回新底部并派发 scroll 事件）读作上滚却不算脱离，流式边界渲染净变矮不丢跟随；向下滚回底部 96px 自动重附；脱离后内容增长不构成脱离信号（突发渲染不丢跟随）；
...
- **历史尾部分页**：首渲染只落最后一页（60 条；页起点对齐用户消息——轮的 assistant→tool 对永不跨页），更早的页经"加载更早消息"按需前插（fragment 单次插入 + scrollHeight 差值补偿滚动锚，阅读位置不动；调用卡→结果的合并由**页内注册表**解析——fragment 构建期间 pane 查询不可达，页内注册 + 跨页对齐共同保证调用与结果永不拆对）；轮次 artifacts 重建仍消费全量快照，不受 DOM 分页影响；
...
| 工具 | 用途 |
|---|---|
| Vite + @vitejs/plugin-react | `src/main.tsx` → 代码切分的内容哈希 chunks：入口 ~149 KB min / ~48 KB gzip 仅装第一方代码；常载第三方代码走四个稳定 `manualChunks` 组（react ~196 / rpc ~116 / radix ~96 / markdown ~70 KB），纯应用改动只需重下入口；highlight.js ~129 KB chunk 启动预取，Files 树 ~132 KB chunk 首次激活加载，xterm ~329 KB chunk 首次创建终端时加载，Settings 对话框 ~33 KB chunk 首次点齿轮加载，单个 CSS；`dynamic import` + `manualChunks` + `cssCodeSplit: false`，文件名带内容哈希 → 服务端 `immutable` 伺服；CI 设 `FLUX_BUNDLE_ANALYZE=1` 产出 rollup treemap 构件（构建期 only，不随包分发） |
...
| vitest + jsdom + RTL | 单元测试。jsdom 缺口在 `src/test/setup.ts` 补齐（ResizeObserver、PointerEvent、pointer-capture、scrollIntoView）；`test:coverage` 出 v8 覆盖率摘要（CI 同跑），阈值即**回归地板**——global 80/75/65（行/函数/分支）、`src/lib/**` 92/85/80、`src/core/**` 72/72/52（wire 翻译主要由 e2e harness 锻炼而非 jsdom）——静默的覆盖率滑坡直接 fail CI |
| e2e harness（`e2e/ui-check.mjs` + `shots.mjs`，CDP 直驱 Chrome，零运行时依赖） | 冒烟 + 可访问性 + 视觉回归三合一：axe-core 以**源码注入**进页面（devDependency 只当源码用），三个面（新对话空态、渲染完成的对话、390×844 移动档）不得有 critical/serious 违例（color-contrast 按文档化设计关闭——faint 声部层级；region 关——外壳不做 landmark 划分）；`shots:update` 把巡览快照进 gitignored `e2e/.baseline/`（会话认可的观感，刻意不是提交进库的设计契约），`shots:check` 重放并逐像素 diff（0.2% 预算 + 热图）——PNG 解码与 diff 跑在已启动的 Chrome 内（`createImageBitmap` + OffscreenCanvas），`document.fonts.ready` + 相位锁定无限动画保证同机重跑 0 px |
...
以下机制是架构层的刻意设计选择。本节是索引与现状说明——设计理由就近写在实现与代码注释里；明确接受的妥协（accepted tradeoffs，`T-xx` 编号全仓引用）见 [decisions.md](decisions.md)：
...
**性能边界**：前端命令式 DOM 层的每帧成本约束见「前端：流式渲染」；后端热路径——provider 的 prefix/suffix 历史序列化缓存、`SseParser` 逐 chunk 状态机、`append_messages` 事务批写、工具顺序单飞。
