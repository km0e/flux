...
- **Single source for schema/parsing**: all 9 built-in tools are field structs; the macro infers JSON Schema from the fields (including doc comments) and `call` auto-deserializes arguments (failure → `CoreError::InvalidArguments`); `#[tool(skip)]` / `#[tool(required)]` / `Vec<T>` inference kept; `#[serde(default)]` fields stay out of `required`.
...
graph TB
 ...
 HOOKS["useEscapeKey"]
 PRIM["ui/ — control primitives (button · fields · badge · spinner, styling authority)<br/>+ Radix wrappers (dialog · dropdown-menu · tooltip · tabs, shadcn conventions)"]
 COMP["App · TopBar · Sidebar (Radix Tabs + search filter + inline rename + row menu)<br/>ChatView · MessageList · ChatInput · UsageStats<br/>Explorer (react-arborist + file icons) · RightDock (file tabs + terminal) · FileTabView · TerminalPanel · FileIcon · ErrorBoundary · Toasts (unified errors)<br/>dialogs/ — registry registration + ConfirmDialog + NewChatDialog (fs browser + kind picker) + QuestionCard<br/>settings/ — SettingsDialog (one dialog, three sections) + Providers / MCP / Skills panels"]
 end
 SVC --> LIB
 SVC --> CORE
 DISP --> SH
